...
module draw_spectrogram #(
    ...
    // FFT wires
    wire fft_clk = s00_axis_aclk;
    logic [31:0] fft1_in_data;               //FFT IP takes 64 bit data
    ...
    reg[31:0] output_record_index;  // Index of next byte in output_buffer to write FFT output to
    ...
    // Hook up FIFO an FFT I/O ports
    always_comb begin
        s00_axis_tready = !fifo_full && reset_counter == 0;
    ...
    assign mem_r_addr = ((x + 2) % 800) * 512 + y;

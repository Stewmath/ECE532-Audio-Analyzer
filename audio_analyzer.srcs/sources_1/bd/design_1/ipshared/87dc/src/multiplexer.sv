...
module multiplexer(
    ...
    input[1:0] output_select, // 0: waveform, 1: pitch graph, 2: spectrogram
    ...
    input spectrogram_reset,
    input[7:0] spectrogram_multiplier
    );
    
    wire [7:0] wave_r, wave_g, wave_b;
    wire [7:0] pitch_r, pitch_g, pitch_b;
    wire [7:0] spect_r, spect_g, spect_b;
    
    wire wave_axis_tready, wave_axis_tvalid, spect_axis_tready, spect_axis_tvalid;
    wire[31:0] wave_axis_tdata, spect_axis_tdata;
    
    // Split sample input to go to waveform and spectrogram
    axis_splitter_0 splitter (
        .in_axis_aclk(smpl_axis_aclk),
        .in_axis_aresetn(smpl_axis_aresetn),
        .in_axis_tready(smpl_axis_tready),
        .in_axis_tdata(smpl_axis_tdata),
        //.in_axis_tlast(smpl_axis_tlast),
        .in_axis_tvalid(smpl_axis_tvalid),
        
        .out1_axis_aclk(smpl_axis_aclk),
        .out1_axis_aresetn(smpl_axis_aresetn),
        .out1_axis_tready(wave_axis_tready),
        .out1_axis_tdata (wave_axis_tdata),
        .out1_axis_tvalid(wave_axis_tvalid),
        
        .out2_axis_aclk(smpl_axis_aclk),
        .out2_axis_aresetn(smpl_axis_aresetn),
        .out2_axis_tready(spect_axis_tready),
        .out2_axis_tdata (spect_axis_tdata),
        .out2_axis_tvalid(spect_axis_tvalid)
    );
    ...
    draw_waveform #(
        ...
        .s00_axis_tready(wave_axis_tready),
        .s00_axis_tdata(wave_axis_tdata),
        .s00_axis_tvalid(wave_axis_tvalid)
    );
    
    draw_spectrogram #(
    ) spectrogram (
        .clk(clk), .pixclk(pixclk),
        .r(spect_r), .g(spect_g), .b(spect_b),
        .x(x), .y(y),
        
        .s00_axis_aclk(smpl_axis_aclk),
        .s00_axis_aresetn(smpl_axis_aresetn),
        .s00_axis_tready(spect_axis_tready),
        .s00_axis_tdata(spect_axis_tdata),
        .s00_axis_tvalid(spect_axis_tvalid),
        
        .reset(spectrogram_reset),
        .multiplier(spectrogram_multiplier)
    );

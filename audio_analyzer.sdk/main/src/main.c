...
// Parameters relating to the pitch filter
#define FILTER_DISTANCE 5 // Check samples this far away
#define FILTER_COUNT 3     // Must have this many matches within FILTER_FREQ_RANGE
#define FILTER_FREQ_RANGE 100 // Range within which frequencies must be in for filtering
...
volatile int phase = 0;                // Keeps track of sample index within the current pitch value
...
// Reset pitch graph (also spectrogram)
void reset_pitch_graph()
{
	is_recording = 0;
...
int pitches_close(int freq1, int freq2)
{
	if (freq1 < freq2)
	{
		int tmp = freq1;
...
void update_filtered_pitch_history()
{
	...
	while (pitch_sent_index <= last_recorded_index - FILTER_COUNT) // TODO
	{
		...
		// Check previous notes were close enough
		int freq = cepstrumArray[pitch_sent_index];
		for (int i=pitch_sent_index-1; i>=pitch_sent_index-FILTER_DISTANCE; i--)
		{
			...
			{
				match_count++;
				freq = cepstrumArray[i];
			}
		}
		// Check next notes are close enough
		freq = cepstrumArray[pitch_sent_index];
		for (int i=pitch_sent_index+1; i>=pitch_sent_index+FILTER_DISTANCE; i++)
		{
			if (i < 0 || i > last_recorded_index)
				continue;
			if (pitches_close(freq, cepstrumArray[i]))
			{
				match_count++;
				freq = cepstrumArray[i];
			}
		...
		if (match_count >= FILTER_COUNT)
			...
			filtered_pitch = 0;
		video_send_pitch(filtered_pitch);
		filteredCepstrumArray[pitch_sent_index++] = filtered_pitch;
	}
	...
	*leds = 0xffffffff;
	...
	for (;;)
	{
		...
		if (pitch_graph_visible)
		{
			...
			if (keysPressed[KEY_SPACE] && !is_recording)
			{
				if (playback_active)
				{
					halt_playback();
		...
		if (keysPressed[KEY_BACKSPACE])
		{
			reset_pitch_graph();
		}

		// Switching input source
		if (keysPressed[KEY_L] || ((buttons_pressed) & BTNL))
			fnSetLineInput();
		else if (keysPressed[KEY_M] || ((buttons_pressed) & BTNR))
			fnSetMicInput();
    ...
    // Infinite loop: Forward audio input to output (this never executes but use this for testing)
    for(;;) {
		s16 l, r;
		adau1761_i2s_read(&l, &r);
		adau1761_i2s_write(l, r);
